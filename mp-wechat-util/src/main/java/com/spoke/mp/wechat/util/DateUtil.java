...
public class DateUtil extends org.apache.commons.lang.time.DateUtils {
    ...
    public static java.sql.Timestamp toSqlTimestamp(String sDate) {
        ...
        if (sDate.length() != DateUtil.DATE_FORMAT_DATEONLY.length() && sDate.length() != DateUtil.DATE_FORMAT_DATETIME.length()) {
            return null;
        }
        return toSqlTimestamp(sDate, sDate.length() == DateUtil.DATE_FORMAT_DATEONLY.length() ? DateUtil.DATE_FORMAT_DATEONLY : DateUtil.DATE_FORMAT_DATETIME);
    ...
    public static void main(String[] args) {
        Date date3 = DateUtil.addMinutes(new Date(), 5);
        System.out.println(new Date().after(DateUtil.addMinute(new Date(), -10)));
        Date date1 = DateUtil.addMinutes(new Date(), -1);
        Date date2 = DateUtil.addMinutes(date1, 5);
        System.out.println(date1);
        System.out.println(date2);
        System.out.println(DateUtil.isBetween(new Date(), date1, date2, DateUtil.COMP_MODEL_TIME));
